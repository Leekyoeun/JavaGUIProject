...
public class DrawBoard extends JPanel {
	...
	public void board(Graphics arg0) {
		...
		for (int i = 1; i <= size.getSize(); i++) {
			...
			arg0.drawLine(size.getCell(), i * size.getCell(), size.getCell() * size.getSize(), i * size.getCell()); // 시작점

			// 세로줄 그리기

			arg0.drawLine(i * size.getCell(), size.getCell(), i * size.getCell(), size.getCell() * size.getSize()); // 시작점
